...
$refurbishButton.Location = New-Object System.Drawing.Point(796, 135)
$refurbishButton.Size = New-Object System.Drawing.Size(160, 38)
$refurbishButton.Anchor = "Top,Right"
...
$hint.Location = New-Object System.Drawing.Point(780, 599)
$hint.AutoSize = $true
            ...
            try {
                ...
                if ($state.total -gt 0) {
                    ...
                    if ($active) {
                        ...
                        $statusLabel.Text = "正在处理第 $displayIndex / $moduleTotal 个：$project"
                    } else {
                        $progress.Style = "Continuous"
                        $progress.MarqueeAnimationSpeed = 0
                        $progress.Maximum = $moduleTotal
                        $progress.Value = $current
                        $statusLabel.Text = "$percent%  $displayPhase（$current / $moduleTotal）"
                    }
                    ...
                    $progressDetail.Text = "本模块已处理 $done 个 · 本模块待处理 $left 个 · $activity · 模块：$displayPhase$eta · 已用 $elapsed 秒"
                } else {
                    $progress.Style = "Marquee"
                    $progress.MarqueeAnimationSpeed = 30
